...
toggleRabbitBtn.addEventListener("click", () => {
  rabbitOn = !rabbitOn;
});
...
function drawOverlay() {
  ...
  const portrait = window.innerWidth <= 768; // スマホ
...
if (santaHatOn && santaHatImg.complete) {
  ...
  const y = overlay.height * (portrait ? -0.05 : 0.0);
...
if (haloOn && haloImg.complete) {
  ...
  const x = (overlay.width - w) / 2;
  const y = overlay.height * (portrait ? -0.1 : -0.12);
